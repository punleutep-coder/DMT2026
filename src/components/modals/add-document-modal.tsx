...
export default function AddDocumentModal({ isOpen, onClose }: AddDocumentModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-lg:w-screen max-lg:h-screen max-lg:max-w-none max-lg:max-h-none max-lg:top-0 max-lg:left-0 max-lg:translate-x-0 max-lg:translate-y-0 max-lg:rounded-none lg:w-[80vw] lg:max-w-6xl lg:h-[90vh] glassmorphic-card p-4 sm:p-10 flex flex-col gap-0 overflow-hidden">
        <DialogHeader className="mb-8 flex-none">
          <DialogTitle className="text-3xl font-bold font-rotanak text-[#000066]">{t('addNewDocument')}</DialogTitle>
        </DialogHeader>
        <Form {...form}>
              ...
              <div className="space-y-10 pb-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="flex flex-col">
                          <FormLabel className="text-[#1D41D5] text-xl font-bold block mb-2">{t('initialReceiver')}</FormLabel>
                          <Combobox
                            ...
                            <FormField 
                              ...
                              render={({ field }) => ( 
                                <FormItem>
                                  ...
                                  <FormControl><Input type="url" placeholder="https://://" {...field} className="h-14 text-xl" /></FormControl>
                                  <FormMessage />
                                </FormItem> 
                              )} 
                            />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {hasPermission(currentUser, 'canEditKeywords') && <FormField control={form.control} name="keywords" render={({ field }) => ( <FormItem><FormLabel className="text-xl font-bold block mb-2">{t('keywords')}</FormLabel><FormControl><Input placeholder={t('keywordsPlaceholder')} {...field} className="h-14 text-xl" /></FormControl><FormMessage /></FormItem> )} />}
                ...
                {hasPermission(currentUser, 'canEditInitialNote') && <FormField control={form.control} name="initialNote" render={({ field }) => ( <FormItem><FormLabel className="text-xl font-bold block mb-2">{t('initialNote')}</FormLabel><FormControl><Textarea {...field} className="min-h-[120px] text-xl" /></FormControl><FormMessage /></FormItem> )} />}
              </div>
            </ScrollArea>
            <DialogFooter className="pt-8 flex-none gap-6">
              <Button type="button" variant="ghost" className="flex-1 h-16 text-xl font-bold" onClick={onClose}>{t('cancel')}</Button>
              <Button 
                type="submit" 
                variant="secondary"
