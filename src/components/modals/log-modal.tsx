  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent 
        ...
        style={{ background: 'rgba(248, 248, 248, 0.25)', borderColor: 'rgba(248, 248, 248, 0.2)' }}
        >
        <DialogHeader>
        ...
        <ScrollArea className="h-[60vh] p-4">
              ...
              <div className="space-y-4 mb-8">
                ...
                <div className="space-y-4">
                    {sourceDocuments.map(sourceDoc => sourceDoc && (
                        <div key={sourceDoc.id} className="relative p-4 bg-muted/30 rounded-lg border-l-4" style={{ borderColor: '#F8F8F8' }}>
                           <h4 className="font-semibold text-foreground mb-1 cursor-pointer hover:underline" onClick={() => handleSourceDocClick(sourceDoc.id, sourceDoc.firestoreId)}>
                             <span style={{ color: '#F8F8F8' }}>{sourceDoc.id}</span> - {sourceDoc.name}
                           </h4>
          ...
          {splitSourceDocument && (
              <div className="space-y-4 mb-8">
                  ...
                  <div className="relative p-4 bg-muted/30 rounded-lg border-l-4" style={{ borderColor: '#F8F8F8' }}>
                      <div className="flex justify-between items-start">
                          <div>
                              <h4 className="font-semibold text-foreground mb-1 cursor-pointer hover:underline" onClick={() => handleSourceDocClick(splitSourceDocument.id, splitSourceDocument.firestoreId)}>
                                  <span style={{ color: '#F8F8F8' }}>{splitSourceDocument.id}</span> - {splitSourceDocument.name}
                              </h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-4">
                {document?.history?.map((entry, index) => (
                  <div key={index} className="p-4 bg-muted/30 rounded-lg border border-border/50">
                    <h4 className="font-bold text-[#F8F8F8] mb-2">{entry.department}</h4>
                    <div className="text-sm space-y-1 text-muted-foreground">
                       ...
                       <p><strong className="text-foreground/80">Period:</strong> <span className="text-[#F8F8F8] font-medium">{formatDuration(entry.start, entry.end)}</span></p>
            ...
            <div className="space-y-6">
              ...
              <div className="relative space-y-6 border-l-2 border-[#F8F8F8]/30 pl-6">
                 {docLogs.length > 0 ? docLogs.map((log, index) => (
                    <div key={index} className="relative">
                        <div className="absolute -left-[30px] top-1.5 h-4 w-4 rounded-full bg-[#F8F8F8] ring-4 ring-background" />
                        <p className="font-medium text-foreground">
                            From <span style={{color: '#F8F8F8'}}>{log.oldStatus}</span> to <span style={{color: '#F8F8F8'}}>{log.newStatus}</span>
                        </p>
